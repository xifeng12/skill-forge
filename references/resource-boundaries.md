...
Scripts generated or used by skills should follow this classification:

| Script Type | Location | Rationale |
|-------------|----------|-----------|
| **Template scripts** | `skill/scripts/` | Part of skill, must persist |
| **Dependency scripts** | `skill/scripts/` | Required for execution |
| **User instance scripts** | `skill/scripts/` (default) | User can override |
| **Temporary scripts** | Working directory or `/tmp` | Ephemeral, discard after use |

### Why skill/scripts/ over working directory

1. **Self-contained**: Scripts travel with skill on migration/share
2. **No pollution**: Does not clutter user's working directory
3. **Version control**: Can archive by date for traceability
4. **Consistency**: Same location regardless of where skill is invoked

### Naming Convention

- `script-name-template.ps1` - Template (read-only)
- `script-name-latest.ps1` - Most recent instance
- `script-name-YYYYMMDD.ps1` - Archived by date

### Anti-pattern

❌ Generating scripts in working directory on every invocation
✅ Check `skill/scripts/` first, reuse or generate once
